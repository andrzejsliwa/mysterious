...
RSpec.describe 'Notes', type: :request do
  ...
  describe 'PATCH /leads/:lead_id/notes/:id.json' do
    let(:new_message) { "new" }
    let(:note) { lead.notes.create(message: "message", details: "details") }
    subject { patch "/leads/#{lead.id}/notes/#{note.id}.json", {note: note.attributes.merge(message: new_message)}, @env }


    context 'as ADMIN' do
      before { login_as "andrzej.sliwa@i-tool.eu", :admin }

      it 'updates note entity' do
        expect { subject; note.reload }.to change { note.message }.to(new_message)
      end
    end

    context "as GUEST" do
      before { login_as "andrzej.sliwa@i-tool.eu", :guest }
